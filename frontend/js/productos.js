...
function createProductLinkElement(product) {
  ...
  const imagenUrl = product.imagen_icono
    ? `http://localhost:3001${product.imagen_icono}`
    : 'https://placehold.co/300x300.png?text=Imagen+no+disponible';

  card.innerHTML = `
    <a href="producto-detalle.html?id=${product.id}" class="product-card__image-container">
      <img src="${imagenUrl}" alt="${product.nombre}" class="product-card__image" onerror="this.onerror=null;this.src='https://via.placeholder.com/300x300.png?text=Imagen+no+disponible';">
      <div class="product-card__overlay">Ver Producto</div>
    </a>
    <div class="product-card__info">
      <p class="product-card__brand">${product.marca || 'Macs'}</p>
